...
//------------ //lbuf[0] == CID_DMX	
          ...
          if (lbuf[1] == SET_VAL_DMX_REQ) {     // Установить значение на одиночное DMX устройство      
            adr = lbuf[4];
            if (adr != 0){
              if (dmx_old.data[adr] < lbuf[5]){ // Увеличение значения DMX устройства
                dmx.direction = 1;   
              }
              else{                           
                dmx.direction = 0;          // Уменьшение значения DMX устройства
              }
              dmx.adr = adr;                // Сохраняем адрес изменяемого DNX устройства
//              dmx_old.data[adr] = dmx.data[adr]; // Сохраняем старое значение изменяемого DNX устройства                 
              dmx.data[adr] = lbuf[5];      // Устанавливаем новое значение DNX устройства
              dmx.smooth_step = lbuf[6];    // Устанавливаем шаг изменения
              dmx.smooth_time = lbuf[7];    // Устанавливаем время между ближайшими шагами
              dmx.flag_send = 1;            // Необходимо отправить данные
            }
            send_simple_answer(conn, CID_DMX, SET_VAL_DMX_ACK, 0, 0);
          }
          else if (lbuf[1] == SET_RGB_DMX_REQ) {   // установить значение на RGB DMX устройство
            adr = lbuf[4];
            if (adr != 0){
