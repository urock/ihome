...
void man()
{
	printf("STMicroelectronics UART Flash Loader command line v2.5.0 \n\n");
	printf(" Usage : \n\n");
	...
	printf("  -o              (Get or Set STM32F1x option bytes: use -d command for others!) \n");              
					...
					{     // STM32 or STR911 is used
						...
						if (STBL_GET_ID(&size,NULL) == SUCCESS)
						{
							...
							if (STBL_GET_ID(&size,pID) == SUCCESS) // Get the ID
							{
								...
								else if((PID == 0x416) || (PID == 0x436)) 
								{
									family = 4; // STM32L is used 
									ADDR_XX_OPB = ADDR_L1_OPB ;
						
								}
								else if(PID == 0x411 || (PID == 0x413)) 
								{
									family = 5; // STM32F2 or F4 is used 
